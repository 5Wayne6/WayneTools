// pages/protractor-tool/protractor-tool.js
Page({
  data: {
    isRelease: true,
    isTake: !1,
  ...
  reset: function () {
    this.bgcanvas.drawImage("../../images/index-icon/BackGround.jpg", 0, 0, 400, 1e3, 0, 0, this.width, this.h),
  ...
  show: function() {
    var t = this;
    // console.log("1", wx.getStorageSync("isAuth"));
    if (wx.getStorageSync("isAuth") !== false) {
        if (wx.getStorageSync("isAuth") === true) {
            ...
            this.drawbg();
            // console.log("2", wx.getStorageSync("isAuth"));
        } 
        else if (wx.getStorageSync("isAuth") === "") {
            wx.authorize({
                scope: "scope.camera",
                success: function() {
                    // console.log("3", wx.getStorageSync("isAuth"));
                    t.setData({
                        show: true,
                        isAuth: true
                    });
                    wx.getSetting({
                        success: function(t) {
                            // console.log("4", wx.getStorageSync("isAuth"));
                            wx.setStorageSync("isAuth", t.authSetting["scope.camera"]);
                        }
                    });
                    t.bgcanvas.clearRect(0, 0, t.width, t.h);
                    t.drawbg();
                },
                fail: function() {
                    // console.log("5", wx.getStorageSync("isAuth"));
        ...
        wx.openSetting({
            success: function(t) {
                // console.log("6", wx.getStorageSync("isAuth"));
                wx.setStorageSync("isAuth", t.authSetting["scope.camera"]);
            }
        });
    }
  ...
  //   var t = this;
  //   console.log("1",wx.getStorageSync("isAuth"))
  //   !1 !== wx.getStorageSync("isAuth") ? 
  //   (!0 === wx.getStorageSync("isAuth") && 
  //   (this.setData({show: !0}), 
  //   this.bgcanvas.clearRect(0, 0, this.width, this.h), 
  //   this.drawbg()), 
  //   console.log("2",wx.getStorageSync("isAuth")),
  //   "" === wx.getStorageSync("isAuth") && 
  //   wx.authorize({
  //     scope: "scope.camera",
  //     success: function () {
  //       console.log("3",wx.getStorageSync("isAuth"))
  ...
  onLoad: function () {
    ...
    t.windowHeight > 1.6 * t.windowWidth ? this.w = .8 * t.windowWidth : this.w = t.windowHeight / 2,
      ...
      this.bgcanvas = wx.createCanvasContext("firstCanvas"), this.bgcanvas.drawImage("../../images/index-icon/BackGround.jpg", 0, 0, 400, 1e3, 0, 0, this.width, this.h),
