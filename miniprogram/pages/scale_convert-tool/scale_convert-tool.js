// pages/scale_convert-tool/scale_convert-tool.js
import * as data from '../../myhelper/others-helper/scale_data'
Page({
  /**
   * 页面的初始数据
   */
  ...
  KeyInput: function (e) {
    ...
    if (this.data.decimal != '')
      ...
      this.data.ASCIInum_show = '显示字符含义';

    if(this.data.decimal != ''){
      if(this.data.decimal < 32 || this.data.decimal >= 127){
        this.data.ASCIInum = '无字符';
      }
      else if(this.data.decimal == 32){
        this.data.ASCIInum = '(Space)';
      }
    }
